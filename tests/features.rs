...
use pebbles_charts::{ChartStyle, annotation, bar_chart, line_chart, series, series_with_errors};
...
fn loading_and_error_states_lay_out() {
    lay_out(
        bar_chart(vec!["A".into()], vec![series("v", vec![1.0])])
            .loading(true)
            .width(300.0)
            .height(200.0),
    );
